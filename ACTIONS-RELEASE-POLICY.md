...
O workflow `http-examples-ci.yml` preserva as validações existentes e passa a ser manual. Execute os validadores locais de manifesto/corpus durante desenvolvimento e dispare o gate remoto apenas se necessário ao fechamento da entrega. Commits/PRs não iniciam CI. A publicação/sincronização documental deve acompanhar o corte do produtor canônico.
